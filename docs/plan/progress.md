...
### バックエンド（`backend/`）
- **CLI / MCP 入口**（2026-09-13）：`backend/cli.py`（`sfx.bat`）と `backend/mcp_server.py`。バックエンド未起動なら detached で自動起動し、生成完了まで待って WAV パスを返す。`POST /api/shutdown` で即時終了、CLI 起動分は 10 分アイドルで自動終了（`--idle-timeout`）。Claude Code には `.mcp.json` で登録済み。動作確認：別ポート（`SFX_PORT=8799`）でコールドスタート→生成→停止、MCP stdio クライアントからのツール呼び出し、`SFX_IDLE_TIMEOUT=20` で生成後に自動終了すること
